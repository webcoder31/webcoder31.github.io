...
function resizeBanner(fullscreen)
{
    ...
    var navbarHeight = $('#section-header .uk-navbar').outerHeight();
    var bannerHeight = $('#banner .banner-head div').outerHeight();
    var tweakersHeight = $('#section-tweakers > .uk-section').outerHeight();
    ...
    if(newHeight != $('#banner').height())
    {
        ...
        if (! fullscreen) {
            var ratio = window.innerWidth < 640 ? 0.8 : 1.2;
...
function resizeStickyPlaceholder(newHeight)
{
    var stickyPlceHolder =  $('#sticky-header-wrapper .uk-sticky-placeholder');
...
function initFXConfigurators(options = {})
{
    var defaults = {
    ...
    function arePointsVisualyClose(point, nextPoint)
    {
        var coords = point.coords.scrCoords;
        var nextCoords = nextPoint.coords.scrCoords;
        var r = 1;
    ...
    sfx.graph.board.create(
            ...
            [
                function(t)
                {
                    var sliders = sfx.tweaker.sliders;
    ...
    sfx.graph.moveGliders = function (t)
    {
        var sliders = sfx.tweaker.sliders;

        var x = t;
        var y = sliders.speed.Value() / Math.abs(sliders.speed.Value());
    ...
    pfx.graph.board.create(
            ...
            [
                function(t)
                {
                    var sliders = pfx.tweaker.sliders;
    ...
    pfx.graph.moveGliders = function (t)
    {
        var sliders = pfx.tweaker.sliders;

        var xStart = 16 * 0.8 * settings.vpOffsetFactorX / 100;
        var yStart = 9 * 0.8 * settings.vpOffsetFactorY / 100;
        var xStart = 16 * 0.8 * settings.vpOffsetFactorX / 100;
        var yStart = 9 * 0.8 * settings.vpOffsetFactorY / 100;

        if (sliders.parallaxFX.Value())
        {
            var xStart = 16 * 0.8 * sliders.parallaxFXGainX.Value() * Math.sin(t * sliders.parallaxFXFrequencyX.Value() + sliders.parallaxFXPhaseX.Value());
            var yStart = 9 * 0.8 * sliders.parallaxFXGainY.Value() * Math.cos(t * sliders.parallaxFXFrequencyY.Value() + sliders.parallaxFXPhaseY.Value());

            var xEnd = pfx.graph.gliders.parallaxStart.X() + 16 * 0.8 * sliders.parallaxFXGainX.Value() * Math.cos(t * sliders.parallaxFXFrequencyX.Value() + sliders.parallaxFXPhaseX.Value());
            var yEnd = pfx.graph.gliders.parallaxStart.Y() + 9 * 0.8 * sliders.parallaxFXGainY.Value() * Math.sin(t * sliders.parallaxFXFrequencyY.Value() + sliders.parallaxFXPhaseY.Value());
    ...
    var hueCurve = cfx.graph.board.create(
            'circle',
            [[0, 0], 0.9],
            {
                strokeColor: '#D6DF22',
    ...
    function hue2HexColor(hue, grayscale)
    {
        function byte2Hex(b)
        {
    ...
    cfx.graph.moveGliders = function(hue, opacity)
    {
        var hexColor = hue2HexColor(hue, cfx.tweaker.sliders.grayscale.Value());
...
$(document).ready(function() {
    ...
    var fxCtrl = initFXConfigurators();

    // Initialize ENDLESS slideshow.
    var slideshow = $('#banner').endless({
    ...
    $('#section-tweakers').on('beforeshow', function () {
            $('#banner').addClass('tweaked');
        });

    $('#section-tweakers').on('shown', function () {
            $(window).resize();
        });

    $('#section-tweakers').on('hidden', function (e) {
    ...
    $('#section-tweakers .widget').on('beforeshow', function (e) {
            e.stopPropagation();
        });

    $('#section-tweakers .widget').on('shown', function (e) {
            e.stopPropagation();
        });

    $('#section-tweakers .widget').on('hidden', function (e) {
            e.stopPropagation();
        });
    ...
    $('#button-card-controllable').get(0).addEventListener("click", function () {
            setTimeout(function() {
                    $('#banner').addClass('tweaked');
                    // $(window).resize();
                }, 250);
